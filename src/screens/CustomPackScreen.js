...
const CustomPackScreen = () => {
  ...
  const fetchWeeklyPackValue = async () => {
    try {
      ...
      if (categoryId) {
        ...
        const weeklyPack = packs.find(p => p.PackType.duration === 'weekly');
        if (weeklyPack) {
          setWeeklyPackValue(weeklyPack.finalPrice);
        }
      }
    } catch (error) {
      console.error('Error fetching weekly pack value:', error);
    }
  };
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      Alert.alert('Failed to add custom pack to cart', 'Please check your connection and try again.');
    } finally {
      setAddingToCart(false);
    }
  };
